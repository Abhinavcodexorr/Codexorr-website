...
export function TestimonialsSection() {
  ...
  return (
    <section ref={sectionRef} className="relative overflow-hidden bg-white py-28 md:py-36">
      <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(ellipse_65%_45%_at_50%_0%,rgba(236,72,153,0.05),transparent),radial-gradient(ellipse_45%_35%_at_5%_95%,rgba(167,139,250,0.05),transparent)]" />
      ...
      <Container className="flex flex-col gap-14 md:gap-16">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          {quotes.map((q, i) => (
            <motion.div key={q.name} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }}
              ...
              <HolographicCard className="flex h-full flex-col" glowColor={q.glow} intensity={7}>
                <div className="relative flex h-full flex-col gap-6 p-8">
                  <Quote className={`h-7 w-7 opacity-50 ${q.accent}`} aria-hidden />
                  ...
                  <div className="flex flex-col gap-1 border-t border-slate-200 pt-5">
                    ...
                    <span className="text-xs text-slate-400">{q.role}</span>
                  </div>
                </div>
              </HolographicCard>
            </motion.div>
